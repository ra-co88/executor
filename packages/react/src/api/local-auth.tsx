...
export const bootstrapLocalAuthToken = (): void => {
  ...
  if (fromOtc) {
    // Fire the exchange; the code is single-use, so strip the query
    // immediately regardless of outcome (a failed exchange falls through to
    // the stored token or the auth gate).
    url!.searchParams.delete("_otc");
    stripQuery();
    void exchangeOtc(fromOtc);
    return;
  }
